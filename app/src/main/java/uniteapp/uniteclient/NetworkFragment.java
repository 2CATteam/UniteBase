...
public class NetworkFragment extends Fragment {
     ...
     */
    public void startDownload(DownloadParameters toDownload, DownloadCallback<JSONObject> context) {
        cancelDownload();
        if (mCallback != null) {
        ...
        mDownloadTask.execute(toDownload);
    }
    ...
    private static class DownloadTask extends AsyncTask<DownloadParameters, Integer, DownloadTask.Result> {
        ...
        DownloadTask(DownloadCallback<JSONObject> callback) {
            setCallback(callback);
        }

        void setCallback(DownloadCallback<JSONObject> callback) {
            mCallback = callback;
        }
         ...
         */
        class Result {
            JSONObject mResultValue;
            Exception mException;
            Result(JSONObject resultValue) {
        ...
        protected void onPostExecute(Result result) {
            if (result != null && mCallback != null) {
                if (result.mException != null) {
                    mCallback.updateFromError(result.mException.getMessage());
         ...
         */
        private JSONObject downloadUrl(DownloadParameters toDownload) throws IOException {
            ...
            try {
                ...
                connection.setRequestMethod(toDownload.getMethod());
                // Already true by default but setting just in case; needs to be true since this request
                // is carrying an input (response) body.
                connection.setDoInput(true);
                if (!toDownload.getMethod().equalsIgnoreCase("GET"))
                {
                    connection.setDoOutput(true);
                    JSONObject toWrite = new JSONObject();
                    try {
                        for (int index = 0; index < toDownload.getParamKeys().size(); ++index)
                        {
                            toWrite.put(toDownload.getParamKeys().get(index), toDownload.getParamValues().get(index));
                        }
